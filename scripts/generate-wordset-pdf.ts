...
console.log(`<!DOCTYPE html><html lang="sr"><head><meta charset="utf-8"><style>
  ...
  * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  ...
  .head { display: flex; align-items: center; gap: 12px; padding-bottom: 12px; margin-bottom: 16px; border-bottom: 3px solid ${LJUB}; }
  .head img { height: 38px; }
  .head .t { font-size: 20px; font-weight: 800; }
  .head .s { font-size: 11px; color: #888; margin-top: 2px; }
  .grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; }
  .card { break-inside: avoid; border: 1.5px solid ${LJUB}; border-radius: 12px; background: ${LJUB_LIGHT}; padding: 9px 11px; min-height: 58px; display: flex; flex-direction: column; justify-content: center; }
  .card .de { font-size: 13px; font-weight: 700; color: ${LJUB}; line-height: 1.25; }
  .card .sr { font-size: 12.5px; color: #333; margin-top: 5px; padding-top: 5px; border-top: 1px dashed #d9caff; }
