...
const KAMPANJA = "b11-ponuda-2026-08-17b";
...
async function buildAudience(): Promise<{ primaoci: Primalac[]; sumnjivi: string[]; vecKontaktirani: number }> {
  ...
  // DRUGI TALAS: sad ciljamo obrnuto - one koji SU vec dobili neku kampanju
  // (vecina 20.07), a NISU dobili jutrosnji b11-ponuda-2026-08-17.
  const jutros = new Set<string>();
  const jutrosPath = path.resolve(__dirname, "_b11_avg_ponuda_sent_2026-08-17.json");
  if (fs.existsSync(jutrosPath)) {
    for (const r of JSON.parse(fs.readFileSync(jutrosPath, "utf-8")) as Array<{ email: string }>) {
      jutros.add(r.email.toLowerCase());
    }
  }

  const primaoci: Primalac[] = [];
  const sumnjivi: string[] = [];
  let vecKontaktirani = 0; // ovde: preskoceni jer su dobili jutros ili nikad nisu kontaktirani
  for (const [email, nivo] of poslednji) {
    if (nivo !== "B1.1" || izbaci.has(email) || jutros.has(email)) { if (jutros.has(email)) vecKontaktirani++; continue; }
    const c = crmPoMejlu.get(email);
    if (!c || !kontaktiraniIds.has(c.id)) continue; // nikad kontaktirani su vec dobili jutros
...
async function run() {
  ...
  console.log(`Publika: ${primaoci.length} (poslednji test = B1.1, RANIJE kontaktirani - drugi talas) | Subject: ${SUBJECT}`);
  console.log(`Preskočeno: ${vecKontaktirani} dobili jutrošnji mejl, ${sumnjivi.length} sumnjiv domen`);
  sumnjivi.forEach((s) => console.log(`  ⚠ ${s}`));

  if (PREVIEW) {
    const f = path.resolve(__dirname, `_b11_avg2_preview.html`);
  ...
  fs.writeFileSync(path.resolve(__dirname, "_b11_avg2_ponuda_2026-08-17.json"), JSON.stringify(primaoci, null, 2));
  ...
  fs.writeFileSync(path.resolve(__dirname, "_b11_avg2_ponuda_sent_2026-08-17.json"), JSON.stringify(poslato, null, 2));
  console.log(`Lista poslatih (sa Resend ID-jevima) u _b11_avg2_ponuda_sent_2026-08-17.json - sledi CRM sinhronizacija.`);
}
