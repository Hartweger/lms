/**
 * Letnja ponuda za A1.2 grupu (start 14.07.2026 u 18h) - dve publike:
 *  - "test": 61 osoba sa rezultatom A1.2 na besplatnom testiranju (bez kupovine)
 *  - "naki": 40 NaKI lidova (stage=nov, level=A1, nisu na test listi) - varijanta
 *    sa pozivom na testiranje umesto "ako imaš pitanje"
 * Tekst odobrila Nataša 14.07.2026.
 *
 *   npx tsx scripts/send-a12-letnja-ponuda.ts            # DRY
 *   npx tsx scripts/send-a12-letnja-ponuda.ts --send
 */
...
const SUBJECT = "Održi jezičku formu tokom leta - A1.2 grupa kreće večeras";
const KURS_URL = "https://www.hartweger.rs/kursevi/grupni-kurs-nemackog-jezika-a1-2-2";
...
function buildEmail(firstName: string | null, variant: "test" | "naki"): string {
  ...
  const zavrsni =
    variant === "test"
      ...
      : `<p style="font-size:15px;line-height:1.6;color:#444;margin:0 0 22px">Ako nisi siguran/na da li je A1.2 pravi nivo za tebe, uradi naše <a href="${TEST_URL}" style="color:#4fb1d3">besplatno testiranje</a> (traje 15 minuta) ili samo odgovori na ovaj mejl pa ćemo te posavetovati.</p>`;
  const footer =
    variant === "test"
...
<span style="display:none;max-height:0;overflow:hidden;color:#f8f9fa">A1.2 grupa kreće večeras u 17h - mala grupa, opušteno, bez stresa.</span>
...
<h1 style="font-size:22px;margin:0 0 16px;color:#1a1a2e">A1.2 grupa kreće večeras u 17:00 🌞</h1>
<p style="font-size:15px;line-height:1.6;color:#444;margin:0 0 16px">${pozdrav}</p>
<p style="font-size:15px;line-height:1.6;color:#444;margin:0 0 18px">znam da je leto vreme kada svi poželimo malo pauze i odmora - ali da nam septembar ne bi bio stresan kad se vratimo svim obavezama, pozivam te da se priključiš našoj <strong>A1.2 grupi koja kreće baš večeras u 17:00</strong>. Tako održavaš jezičku formu tokom leta, uz najpovoljniju cenu i malu grupu u kojoj ćaskaš opušteno, kao sa prijateljima - bez stresa.</p>
<div style="background:#fff8f3;border-left:3px solid #e8915a;border-radius:6px;padding:16px 18px;margin:0 0 18px"><div style="font-size:15px;line-height:1.8;color:#1a1a2e"><strong>Šta te čeka:</strong><br>• živa online nastava sa profesorkom, utorkom i četvrtkom 17:00-18:00<br>• mala grupa, najviše 6 polaznika - svako dobija reč na svakom času<br>• 7 nedelja nastave (do 27.08) + video lekcije, vežbe i testovi na platformi, dostupni 24/7<br>• sertifikat po položenom završnom ispitu</div></div>
...
${zavrsni}
<p style="font-size:15px;line-height:1.6;color:#444;margin:0">Vidimo se večeras?</p>
...
async function run() {
  const input = JSON.parse(fs.readFileSync(path.resolve(__dirname, "_a12_letnja_ponuda_2026-07-14.json"), "utf-8"));
  ...
  for (const { p, variant } of sviRed) {
    const res = await fetch("https://api.resend.com/emails", {
      ...
      headers: {
        ...
        "Idempotency-Key": `a12-letnja-2026-07-14/${p.email}`,
      },
      body: JSON.stringify({
  ...
  if (sentCrmIds.length) {
    fs.writeFileSync(path.resolve(__dirname, "_a12_letnja_sent_crm_ids.json"), JSON.stringify(sentCrmIds, null, 2));
    console.log(`CRM id-jevi poslatih NaKI lidova u _a12_letnja_sent_crm_ids.json (${sentCrmIds.length}).`);
  }
}
