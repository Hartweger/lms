...
const CSP = [
  ...
  // 25.07: + gr (Sentry prijava b8d990e0, posetilac iz Grčke - letovanje/dijaspora) + cy/pt/lu/ie/fi
  // preventivno (ista logika: letovališta i preostale EU zemlje sa našom publikom)
  // 11.09: + com.ua (Sentry prijava ae2be50d, posetilac iz Ukrajine preko ManyChat linka na /naki,
  // Android WebView) + pl/cz/sk/hu/ro/bg/ru/com.au/ca preventivno - susedi, EU i dalja dijaspora;
  // isti GA4 Google signals ping (ads/ga-audiences), bez ovoga se samo gubi remarketing signal
  // za tog posetioca, stranica radi normalno.
  // 15.08: + Google Ads domeni. AW tag je pušten 11.08 (layout.tsx gtag config + PixelPurchase
  // conversion), ali mu CSP blokira SVAKI beacon - u konzoli na svakoj strani stoji
  // "Refused to connect ... pagead2.googlesyndication.com/ccm/collect ... en=page_view".
  // Znači ni page_view ni Purchase konverzija nisu stizali Google-u, pa Smart Bidding i
  // dalje nije imao signal mimo GA4 - upravo ono zbog čega je tag i dodavan.
