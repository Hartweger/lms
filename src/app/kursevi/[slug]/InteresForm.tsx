...
export default function InteresForm({ nivo }: { nivo: string }) {
  const uid = useId();
  const imeId = `interes-ime-${uid}`;
  const emailId = `interes-email-${uid}`;
  ...
  return (
    <form onSubmit={submit} className="flex flex-col gap-3 w-full max-w-sm">
      <label htmlFor={imeId} className="sr-only">Ime</label>
      <input
        id={imeId}
        type="text" placeholder="Ime" value={ime} onChange={(e) => setIme(e.target.value)}
        className="border border-gray-300 rounded-lg px-4 py-3"
      />
      <label htmlFor={emailId} className="sr-only">Mejl adresa</label>
      <input
        id={emailId}
