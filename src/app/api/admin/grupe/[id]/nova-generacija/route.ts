...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: g } = await admin
    .from("groups")
    .select("id, level, days, session_time, duration_weeks, start_date, professor_id, professor:professor_id(full_name)")
    .eq("id", id)
    .single();
  ...
  try {
    gas = await callGas("openTerm", {
      nivo: g.level, prof: profIme, days: g.days, time: g.session_time, weeks: g.duration_weeks, startDate: g.start_date,
    });
  } catch (e) {
